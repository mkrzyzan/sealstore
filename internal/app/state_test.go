...
// validPayTxs returns well-formed marshaled payment txs from the account at
// address `addr` — enough for mempool-style validity checks, which only require
// a successful Parse.
func validPayTxs(addr string) [][]byte {
	body := tx.Payment{From: []byte(addr), Seq: 1, Transfers: []tx.Transfer{{To: []byte(addr), Amount: 1}}, Fee: 0}
	return [][]byte{
		payCommitTx(addr, [32]byte{}, 100, 0),
		payRevealTx(body, make([]byte, 32), make([]byte, 32), make([]byte, 32), 100),
	}
}
...
func TestIsValid(t *testing.T) {
	...
	for _, raw := range validPayTxs(addrOf(hash([]byte("mempool-seed")))) {
	...
	invalid := [][]byte{
		...
		[]byte{byte(tx.TxPayCommit)},            // tag only, no payload
		[]byte{byte(tx.TxPayCommit), 0x01, 'k'}, // acct but truncated commitment
		[]byte{byte(tx.TxCommit), 0x01, 'k'},    // removed seal tag must stay unknown
		[]byte{byte(tx.TxReveal), 0x01, 'k'},    // removed seal tag must stay unknown
...
func TestMalformedRejected(t *testing.T) {
	...
	for _, raw := range [][]byte{
		[]byte("nonsense"),                      // invalid type tag
		[]byte{byte(tx.TxPayCommit)},            // tag only, no payload
		[]byte{byte(tx.TxPayCommit), 0x05, 'k'}, // acct length prefix exceeds remaining
	} {
...
func TestCheckTxValidates(t *testing.T) {
	...
	for _, raw := range validPayTxs(addrOf(hash([]byte("checktx-seed")))) {
